...
export const ScheduleTable: React.FC<ScheduleTableProps> = ({
  ...
  return (
    <div className="schedule-table-wrapper" onKeyDown={handleKeyDown} tabIndex={0} onMouseUp={handleMouseUp}>
      {/* Toolbar */}
      ...
      {/* Color picker popup (drag release or right-click drag) */}
      {showColorPicker && (
        <div className="color-picker-popup" style={{ left: popupPos.x, top: popupPos.y }}
          ...
          <button className="color-cancel-btn" onClick={handleClearColor}>清除底色</button>
          <button className="color-cancel-btn" onClick={() => setShowColorPicker(false)} style={{ marginLeft: 8 }}>取消</button>
        </div>
      )}
    </div>
  );
